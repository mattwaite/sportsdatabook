...
So I may have been too hasty with my damning of the waffle chart library and it's handling of finishing touches. It turns out it's not *that* bad, but it's not great. The reason it's not that bad is that it *is* actually using ggplot under the hood. The bad part is -- it's doing things to ggplot that aren't clear, so what you expect to work won't work exactly. 
...
Truth is, I'm not sure what the hell is going on with the sizing. You can try it and you'll find that the outputs are ... unpredictable. 
...
iron(
 ...
 waffle(
   ...
   colors = c("black", "red")) + labs(caption="Source: NCAA | Graphic by Matt Waite")
) 
```
