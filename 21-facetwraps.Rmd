...
```
```{r}
logs <- read_csv("~/Dropbox/SPMC350-Data-Literacy-and-Analytics-in-Sports/Data/biglogs.csv")
```
...
The first thing we can do is look at a line chart, like we have. 
...
```{r}
big10 %>% summarize(mean(TeamFGPCT))
```
