...
```{r}
oppwinlosslogs %>% group_by(Opponent) %>% summarize(games=n(), wins=sum(winloss)) %>% mutate(winpct = wins/games) -> opprecord
```
...
```{r}
winswithopppct %>% filter(winpct > .6) %>% mutate(week = floor_date(Date, unit="weeks")) %>% group_by(week, Conference) %>% summarize(wins=sum(winloss)) %>% filter(Conference %in% p5) -> qualitywins
```
