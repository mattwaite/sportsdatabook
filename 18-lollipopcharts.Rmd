...
```{r}
conferenceshooting <- logs %>% group_by(Conference) %>% summarize(totalshots = sum(TeamFG), totalattempts = sum(TeamFGA)) %>% mutate(conferenceshootingpct = totalshots/totalattempts)
```

Now I'm going to do the same with teams. 

```{r}
teamshooting <- logs %>% group_by(Team, Conference) %>% summarize(totalshots = sum(TeamFG), totalattempts = sum(TeamFGA)) %>% mutate(teamshootingpct = totalshots/totalattempts)
```
...
```{r}
giveandtake <- winlosslogs %>% group_by(Conference, Team) %>% summarize(turnovers = sum(TeamTurnovers), assists = sum(TeamAssists), wins=sum(winloss)) 
```
